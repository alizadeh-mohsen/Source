...
namespace Roham
{
    public partial class frmBase : Form
    {
        ...
        private void DisplayData()
        {
            ...
            {
                SqlCommand command = new SqlCommand("SelectBase", connection)
                {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            if (txtUnit.Text != "")
            {
                using (SqlConnection connection = DbHelper.GetConnection())
                {
                    SqlCommand command = new SqlCommand("InsertBase", connection)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (txtUnit.Text != "")
            {
                using (SqlConnection connection = DbHelper.GetConnection())
                {
                    SqlCommand command = new SqlCommand("UpdateBase", connection)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridView1.Columns["delete"].Index)
            {
                ...
                if (ID != 0)
                {
                    using (SqlConnection connection = DbHelper.GetConnection())
                    {
                        SqlCommand command = new SqlCommand("DeleteBase", connection)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
                        command.Parameters.AddWithValue("@id", ID);
